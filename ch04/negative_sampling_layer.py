...
class UnigramSampler:
    ...
    def get_negative_sample(self, target):
        ...
        if not GPU:
            ...
            # speed has priority when calculate with GPU(cupy)
            # there are some cases which negative samples includes target
            negative_sample = np.random.choice(
                self.vocab_size, size=(batch_size, self.sample_size),
...
class NegativeSamplingLoss:
    ...
    def forward(self, h, target):
        ...
        # forward script of positive examples
        score = self.embed_dot_layers[0].forward(h, target)
        correct_label = np.ones(batch_size, dtype=np.int32)
        loss = self.loss_layers[0].forward(score, correct_label)

        # forward script of negative examples
